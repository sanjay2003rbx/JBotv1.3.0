...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>stats' then
		message.channel:sendMessage(string.format(' ``` Name: JBot#7214. Bot: Yes. Desc: Multipurpose bot used to moderate servers or provide fun commands for you and your server. Language: Lua. Framework: Luvit. Lib: Discordia. Lib Creator: Sinister Rectus. Region: World. Version: 1.3.0. Updated: Yes. Updates: No. ```, %s', message.author:getMentionString()))
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	if cmd == '>warn' then
		for _, member in pairs(message.mentions.members) do
		message.channel:sendMessage(string.format('%s has been warned!', message.author:getMentionString()))
			end
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>cmds' then
		message.channel:sendMessage(string.format(' ``` Comamnds: @Creator: >test, >servers, >stats, >disconnect, >todo @Admin: >kick <@user>, >ban <@user>, >warn <@user> >admins, >creator @Free: >cmds, >joke, >invite, prefix, >about, >home, >new ```, %s', message.author:getMentionString()))
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>new' then
		message.channel:sendMessage(string.format(' ``` Current Version 1.3.0: Added "=todo" for list of random stuff. Added >warn <@user> to warn user. Fixed kicked bug. ```, %s', message.author:getMentionString()))
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == 'prefix' then
		message.channel:sendMessage(string.format(' ``` The prefix is a ">" also known universally as an error sign! Chat >cmds for a list of commands.  ```, %s', message.author:getMentionString()))
